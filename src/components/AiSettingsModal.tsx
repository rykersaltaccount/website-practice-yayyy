      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-xl border border-white/[0.12] bg-[#0c0d12] shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[180px_1fr]">
          ...
          <form onSubmit={save} className="space-y-4 p-6">
            ...
            <div className="flex items-center justify-between pt-2"><p className="max-w-xs text-[10px] leading-relaxed text-[#62666f]">Keys are stored in this browser's local storage. Never use a service-role key in a browser app.</p><div className="flex items-center gap-3">{saved && <span className="text-[11px] font-semibold text-[#10b981]">Saved</span>}<button type="submit" className="linear-btn-primary px-4 py-2 text-xs font-semibold">Save {tasks.find(item => item.id === task)?.label}</button></div></div>
          </form>
        </div>
      </div>
    </div>
