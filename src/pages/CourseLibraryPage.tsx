...
const CourseLibraryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => setElapsedSeconds(Math.floor((Date.now() - generationStartedAt) / 1000)), 1000);
  ...
  const createCourse = async (event: React.FormEvent) => {
    ...
    setError('');
    setGenerationStage('Validating course generation settings...');
    setGenerationStartedAt(Date.now());
    try {
      const generated = await generateCourseSyllabus(topic.trim(), level, setGenerationStage);
      if (generated) {
        addCourse(generated);
        setShowForm(false);
        setTopic('');
